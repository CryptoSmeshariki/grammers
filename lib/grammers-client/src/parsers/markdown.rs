...
use super::common::{
    after, before, inject_into_message, telegram_string_len, Segment, MENTION_URL_PREFIX, EMOJI_PREFIX
};
...
pub fn parse_markdown_message(message: &str) -> (String, Vec<tl::enums::MessageEntity>) {
    ...
    Parser::new(message).for_each(|event| match event {
        ...
        Event::Start(Tag::Link(_kind, url, _title)) => {
            ...
            } else if url.starts_with(EMOJI_PREFIX) {
                let document_id = url[EMOJI_PREFIX.len()..].parse::<i64>().unwrap();
                entities.push(
        ...
        Event::End(Tag::Link(_kindd, url, _title)) => {
            ...
            } if url.starts_with(EMOJI_PREFIX) {
